...
export default function CreateEmployee() {
  ...
  const { employees, skillDefs, addEmployee, nextEmployeeId } = useData()
  ...
  function handleSkillsChange(names: string[]) {
    ...
    let next = skills.filter(s => !removed.includes(s.name))
    added.forEach(n => next.push({ name: n, rating: 5 }))
    setSkills(next)
  }

  function setRating(name: string, rating: number) {
    setSkills(prev => prev.map(s => s.name === name ? { ...s, rating } : s))
  }
  ...
  function submit(e: React.FormEvent) {
    ...
    setSaved(true)
    setTimeout(() => navigate(`/employees/${id}`), 800)
  }
